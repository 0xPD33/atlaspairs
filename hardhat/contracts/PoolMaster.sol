...
contract PoolMaster is Ownable, ReentrancyGuard {
    ...
    uint256 public bettingPhaseDuration = 1 * 60; // 1 minutes
    uint256 public battlingPhaseDuration = 2 * 60; // 2 minutes
    ...
    function stake(
        ...
        require(
            stakersMapping[msg.sender].stakerAddress == address(0),
            "User already staked for this epoch"
        );
    ...
    function endEpoch(uint256 _poolWinnerId) public onlyOwner nonReentrant {
        ...
        uint256 _poolLoserLength = _poolLoserId == 0
            ? stakersPool1.length
            : stakersPool2.length;
        uint256 _poolWinnerLength = _poolWinnerId == 0
            ? stakersPool1.length
            : stakersPool2.length;
        ...
        for (uint256 i = 0; i < _poolLoserLength; ) {
            ...
            uint256 _userLoseAmount = (_staker.amount * loserTokensPercent) /
                10_000;
        ...
        uint256 _tokenWinningTokenPool = (_tokenLostAmount *
            winnerTokensPercent) / 10_000;
        uint256 _usdcWinningTokenPool = (_usdcLostAmount *
            winnerTokensPercent) / 10_000;
        ...
        if (pools[_poolWinnerId].tokenStakers > 0) {
            _tokenUserWinAmount =
                _tokenWinningTokenPool /
                pools[_poolWinnerId].tokenStakers;
        }
        if (pools[_poolWinnerId].usdcStakers > 0) {
            _usdcUserWinAmount =
                _usdcWinningTokenPool /
                pools[_poolWinnerId].usdcStakers;
        }

        for (uint256 i = 0; i < _poolWinnerLength; ) {
            Staker memory _staker = _poolWinnerId == 0
                ? stakersPool1[i]
                : stakersPool2[i];
            if (_staker.isUsdc) {
                usdc.transfer(
                    _staker.stakerAddress,
                    _staker.amount + _usdcUserWinAmount
                );
                winAddresses.push(
                    WinnerAddress(
                        _usdcUserWinAmount,
                ...
                token.transfer(
                    _staker.stakerAddress,
                    _staker.amount + _tokenUserWinAmount
                );
                winAddresses.push(
                    WinnerAddress(
                        _tokenUserWinAmount,
        ...
        // 0.5% geburned
        uint256 _tokenBurnAmount = (_tokenLostAmount * burnedTokensPercent) /
            10_000;
        uint256 _usdcBurnAmount = (_usdcLostAmount * burnedTokensPercent) /
            10_000;
        usdc.transfer(
        ...
        uint256 _tokenPool3Amount = (_tokenLostAmount * pool3TokensPercent) /
            10_000;
        uint256 _usdcPool3Amount = (_usdcLostAmount * pool3TokensPercent) /
            10_000;
        ...
        for (uint256 i = 0; i < _pool3Length; ) {
            ...
            if (_staker.isUsdc) {
                usdc.transfer(
                    _staker.stakerAddress,
                    _staker.amount + _usdcUserPool3Amount
                );
            } else {
                token.transfer(
                    _staker.stakerAddress,
                    _staker.amount + _tokenUserPool3Amount
                );
            }
    ...
    function getStakedTokensForAddress(
        address _user
    ...
    function getWinnerAddressStakerAddressForLastSeconds(
        uint256 _lastSeconds
    ...
    function getWinnerAddressIsUsdcForLastSeconds(
        uint256 _lastSeconds
    ...
    function setBettingPhaseDuration(
        uint256 _bettingPhaseDuration
    ) public onlyOwner {
        bettingPhaseDuration = _bettingPhaseDuration;
    }

    function setBattlingPhaseDuration(
        uint256 _battlingPhaseDuration
    ) public onlyOwner {
        battlingPhaseDuration = _battlingPhaseDuration;
    }

    function setLoserTokensPercent(
        uint256 _loserTokensPercent
    ) public onlyOwner {
        loserTokensPercent = _loserTokensPercent;
    }

    function setWinnerTokensPercent(
        uint256 _winnerTokensPercent
    ) public onlyOwner {
        winnerTokensPercent = _winnerTokensPercent;
    }

    function setBurnedTokensPercent(
        uint256 _burnedTokensPercent
    ) public onlyOwner {
        burnedTokensPercent = _burnedTokensPercent;
    }

    function setPool3TokensPercent(
        uint256 _pool3TokensPercent
    ) public onlyOwner {
